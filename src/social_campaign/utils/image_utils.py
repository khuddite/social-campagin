...
def remove_white_background(img: Image.Image, threshold: int = 240) -> Image.Image:
    ...
    Pixels where R, G, and B are all above ``threshold`` become fully transparent.
    Others remain fully opaque. This gives a clean cutout from a white-background
    product photo generated by DALL-E 3.
    """
    img = img.convert("RGBA")
    arr = np.array(img)
    # Pixels where all RGB channels exceed the threshold → transparent
