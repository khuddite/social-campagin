...
def _build_background_prompt(
    *,
    ...
    return (
        ...
        f"COLOR HARMONY (critical): The brand colors are {', '.join(brand_colors)}. "
        f"The product itself is predominantly these colors. The background MUST complement them — "
        f"use analogous or contrasting tones that make a {', '.join(brand_colors)} product pop. "
        f"Dark backgrounds work well for bright products. Weave brand colors into the lighting/gels.\n"
